...
Das Uploadkonzept definiert, wie Datei-, Ordner-, Webpage- und Editor-Uploads angenommen, normalisiert, gespeichert und einem Projekt zugeordnet werden.
...
- `projekte.html` bietet Projektanlage mit Datei- und Webpage-Upload.
- `projects.js` bereitet `FormData`, relative Pfade und Ordnerpfade vor.
- `create_project.php` verarbeitet klassische Uploads.
- `create_code_project.php` erzeugt Webpage-Projekte direkt aus Starter-Code.
- `update_project_code.php` speichert spaeter Editor-Dateien und Binaerassets.
...
- Uploads funktionieren einfach fuer Schueler.
- Dateien werden mit zufaelligen Servernamen gespeichert.
- Sichtbare Pfade bleiben in `project_files.relative_path` erhalten.
- Webpage-Projekte brauchen eine `index.html`.
- Fehler hinterlassen keine halben Projekte oder verwaiste Dateien.
...
- `public/html/projekte.html`
- `public/js/projects.js`
- `php/create_project.php`
- `php/create_code_project.php`
- `php/update_project_code.php`
- `uploads/projects`
- `project_files` und `project_folders`.
...
1. Nutzer waehlt Titel, Typ, Sichtbarkeit und Dateien.
2. Frontend normalisiert sichtbare Pfade und sendet `FormData`.
3. Backend prueft Login, CSRF, Typ, Titel, Limit und Dateien.
4. Dateien werden in `uploads/projects` verschoben oder geschrieben.
5. Datenbankeintraege verbinden Projekt, Dateien und Ordner.
6. Antwort aktualisiert die Projektliste.
...
- Serverseitige Pfadbereinigung ist verbindlich.
- Originaldateinamen werden nicht als physische Namen genutzt.
- Uploadordner bleibt statisch gesperrt.
- Groessenlimit kommt aus `settings.json`.
- Webpage-Uploads gelten als aktive, nicht vertrauenswuerdige Inhalte.
...
- Einzeldatei hochladen und herunterladen.
- Webpage mit `index.html`, CSS und JS hochladen.
- Webpage ohne `index.html` ablehnen.
- Sonderzeichen und `..` in Dateinamen testen.
- Editor-Asset speichern und ueber Site-Route laden.
...
- Dateianzahl-Limit.
- Upload-Fortschritt.
- Virenscan oder Quarantaene.
- Bereinigung verwaister Dateien.
- Bessere Diagnose fuer PHP-Postlimits.
...
Uploadkonzept fuer Liveserver beschreibt einen konkreten Baustein von Liveserver. Entscheidend ist, dass Bedienkomfort, Datenmodell, Rechtepruefung und Betrieb zusammenpassen. Wenn diese Grenzen sauber bleiben, kann Liveserver weiter wachsen, ohne unuebersichtlich oder unsicher zu werden.

## 15. Detaillierte technische Leitlinien

Fuer `Upload` gilt, dass jede technische Aenderung zuerst aus der bestehenden Liveserver-Struktur heraus gedacht werden muss. Die Anwendung ist absichtlich einfach aufgebaut: HTML-Dateien liefern die Oberflaeche, JavaScript ruft PHP-Endpunkte auf, PHP prueft Berechtigungen und SQLite speichert den fachlichen Zustand. Diese Einfachheit ist ein Vorteil, solange neue Funktionen nicht an den vorhandenen Helfern vorbeigebaut werden.
...
Eine sinnvolle Teststrategie fuer `Upload` besteht aus positiven und negativen Tests. Positive Tests pruefen, dass der normale Nutzerfluss funktioniert. Negative Tests pruefen, dass unerlaubte Zugriffe scheitern. Negative Tests sind hier besonders wichtig, weil die Anwendung viele direkte PHP-Endpunkte hat.
...
Fuer den Betrieb ist `Upload` nur dann sauber, wenn Dokumentation und ausgelieferter Stand zusammenpassen. Die Version in `config/settings.json`, README und Asset-Querystrings sollte synchron sein. Wenn ein Konzeptverhalten geaendert wird, sollte entweder die passende Konzeptdatei oder die README angepasst werden. Wenn sich Sicherheitsverhalten aendert, muss das Security-Konzept mitgezogen werden.
