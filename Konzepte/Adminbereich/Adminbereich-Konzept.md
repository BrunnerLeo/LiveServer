# Adminbereich-Konzept fuer Liveserver
...
Der Adminbereich soll Systemstatus, Benutzer, Projekte, Speicher, Logs und Konfiguration verwaltbar machen.
...
- `health.php` existiert.
- `logs/api-error.log` existiert.
- `users.role` bereitet Admin vor.
- Projekt- und Nutzerdaten liegen strukturiert vor.
- Kein vollstaendiges Admin-UI vorhanden.
...
- Zukuenftig `admin.html`
- Zukuenftig `php/admin_*.php`
- `database_health`
- `count_users`
- `count_projects`
- `audit_events`.
...
1. Admin loggt ein.
2. Rolle wird geprueft.
3. Dashboard laedt Kennzahlen.
4. Aktion wird per POST gesendet.
5. Backend prueft Adminrecht.
6. Audit-Log schreibt Ereignis.
...
- Admin-API serverseitig sperren.
- Keine Secrets in `settings.json`.
- Kritische Aktionen bestaetigen.
- HTTPS nutzen.
- Logs nicht oeffentlich ausgeben.
...
- Nicht-Admin 403.
- Admin sieht Health.
- Rollenwechsel validiert.
- Projektloeschung auditierbar.
- Ungueltige Config abgelehnt.
...
Adminbereich-Konzept fuer Liveserver beschreibt einen konkreten Baustein von Liveserver. Entscheidend ist, dass Bedienkomfort, Datenmodell, Rechtepruefung und Betrieb zusammenpassen. Wenn diese Grenzen sauber bleiben, kann Liveserver weiter wachsen, ohne unuebersichtlich oder unsicher zu werden.

## 15. Detaillierte technische Leitlinien

Fuer `Adminbereich` gilt, dass jede technische Aenderung zuerst aus der bestehenden Liveserver-Struktur heraus gedacht werden muss. Die Anwendung ist absichtlich einfach aufgebaut: HTML-Dateien liefern die Oberflaeche, JavaScript ruft PHP-Endpunkte auf, PHP prueft Berechtigungen und SQLite speichert den fachlichen Zustand. Diese Einfachheit ist ein Vorteil, solange neue Funktionen nicht an den vorhandenen Helfern vorbeigebaut werden.
...
Eine sinnvolle Teststrategie fuer `Adminbereich` besteht aus positiven und negativen Tests. Positive Tests pruefen, dass der normale Nutzerfluss funktioniert. Negative Tests pruefen, dass unerlaubte Zugriffe scheitern. Negative Tests sind hier besonders wichtig, weil die Anwendung viele direkte PHP-Endpunkte hat.
...
Fuer den Betrieb ist `Adminbereich` nur dann sauber, wenn Dokumentation und ausgelieferter Stand zusammenpassen. Die Version in `config/settings.json`, README und Asset-Querystrings sollte synchron sein. Wenn ein Konzeptverhalten geaendert wird, sollte entweder die passende Konzeptdatei oder die README angepasst werden. Wenn sich Sicherheitsverhalten aendert, muss das Security-Konzept mitgezogen werden.
