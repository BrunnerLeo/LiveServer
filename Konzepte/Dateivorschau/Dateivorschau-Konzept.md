# Dateivorschau-Konzept fuer Liveserver
...
Dateivorschau soll geeignete Projektdateien im Browser anzeigen, ohne unsichere Dateitypen direkt auszufuehren.
...
- Webpage-Projekte haben Hosting.
- Datei-Projekte haben Downloaddetails.
- Projektkarten haben Dateiexplorer.
- Editor laedt Textdateien bis 1 MB.
- Dedizierter Preview-Endpunkt fehlt.
...
- PDF, Bilder, Text sicher ansehen.
- HTML/PHP/Shell nicht blind inline.
- Gleiche Rechte wie Download.
- Download bleibt erhalten.
- Vorschau klar von Webpage-Hosting trennen.
...
- Zukuenftig `preview_project_file.php`
- `find_project_file`
- `user_can_access_project`
- MIME-Allowlist
- Projektkarte/Detailseite.
...
1. Datei erkennen.
2. Preview-Button zeigen.
3. Preview-Endpunkt prueft Recht.
4. Inline streamen.
5. Nicht erlaubte Typen Download-only.
...
- SVG vorsichtig behandeln.
- Text escaped oder plain.
- nosniff setzen.
- Groesse begrenzen.
...
- PDF inline.
- Bild inline.
- Text ohne HTML-Ausfuehrung.
- HTML nicht als Portalcode.
- Fremder Nutzer abgelehnt.
...
- Thumbnails.
- Syntaxhighlighting escaped.
- PDF-Metadaten.
- Preview-Cache.
- Assetvorschau im Editor.
...
Dateivorschau-Konzept fuer Liveserver beschreibt einen konkreten Baustein von Liveserver. Entscheidend ist, dass Bedienkomfort, Datenmodell, Rechtepruefung und Betrieb zusammenpassen. Wenn diese Grenzen sauber bleiben, kann Liveserver weiter wachsen, ohne unuebersichtlich oder unsicher zu werden.

## 15. Detaillierte technische Leitlinien

Fuer `Dateivorschau` gilt, dass jede technische Aenderung zuerst aus der bestehenden Liveserver-Struktur heraus gedacht werden muss. Die Anwendung ist absichtlich einfach aufgebaut: HTML-Dateien liefern die Oberflaeche, JavaScript ruft PHP-Endpunkte auf, PHP prueft Berechtigungen und SQLite speichert den fachlichen Zustand. Diese Einfachheit ist ein Vorteil, solange neue Funktionen nicht an den vorhandenen Helfern vorbeigebaut werden.
...
Eine sinnvolle Teststrategie fuer `Dateivorschau` besteht aus positiven und negativen Tests. Positive Tests pruefen, dass der normale Nutzerfluss funktioniert. Negative Tests pruefen, dass unerlaubte Zugriffe scheitern. Negative Tests sind hier besonders wichtig, weil die Anwendung viele direkte PHP-Endpunkte hat.
...
Fuer den Betrieb ist `Dateivorschau` nur dann sauber, wenn Dokumentation und ausgelieferter Stand zusammenpassen. Die Version in `config/settings.json`, README und Asset-Querystrings sollte synchron sein. Wenn ein Konzeptverhalten geaendert wird, sollte entweder die passende Konzeptdatei oder die README angepasst werden. Wenn sich Sicherheitsverhalten aendert, muss das Security-Konzept mitgezogen werden.
