# Sicherheitskonzept fuer Liveserver
...
Dieses Konzept beschreibt die Sicherheitsgrenzen von Liveserver: Login, Sessions, CSRF, Projektzugriff, Uploadschutz, gehostete Webpages, Editorrechte und Serverbetrieb.
...
- Apache/PHP-Anwendung mit statischem Frontend und JSON-Endpunkten.
- SQLite-Datenbank unter `database/liveserver.sqlite`.
- Uploads unter `uploads/projects` mit gesperrtem Direktzugriff.
- Projektzugriff ueber `private`, `shared`, `public`, `read` und `write`.
- Gehostete Webpage-Projekte laufen ueber `site.php` und sind die wichtigste Sicherheitskante.
...
- Private Daten bleiben nur fuer berechtigte Nutzer sichtbar.
- Alle schreibenden Aktionen verlangen Login, Berechtigung und CSRF.
- Upload-Dateien werden nie direkt statisch ausgeliefert.
- Debugdetails bleiben in Produktion verborgen.
- Hosted-Webpages werden langfristig von der Portal-Origin getrennt.
...
- `php/db_connect.php` fuer Sessions, DB, Rechte und Hilfsfunktionen.
- `php/view_project.php` und `site.php` fuer Webpage-Auslieferung.
- `php/download_project.php` fuer kontrollierte Downloads.
- `config/settings.json` fuer Security-, DB- und Uploadoptionen.
- `.htaccess` in `database`, `logs` und `uploads`.
...
1. Session starten.
2. Requestmethode und CSRF pruefen, wenn Daten geaendert werden.
3. Aktuellen Nutzer laden.
4. Projekt- oder Rollenrecht serverseitig pruefen.
5. Aktion ausfuehren und sichere Antwort liefern.
...
- Frontendzustand ist nie eine Sicherheitsentscheidung.
- Prepared Statements sind fuer SQL Pflicht.
- Dateipfade werden per Normalisierung und `realpath` abgesichert.
- Passwort-Hashes, Tokens und interne Pfade werden nicht ausgegeben.
- Same-Origin-Risiko der gehosteten Webpages muss aktiv beachtet werden.
...
- Direktzugriff auf `uploads`, `database`, `logs` ist gesperrt.
- Private Projekte sind fuer Fremde nicht erreichbar.
- POST ohne CSRF wird abgelehnt.
- Public-Read erlaubt Lesen, aber nicht Schreiben.
- Shared-Write darf nur Webpage-Editor speichern, nicht fremde Metadaten aendern.
...
- Separate Origin fuer gehostete Webseiten.
- Audit-Log fuer sicherheitsrelevante Aktionen.
- Rate-Limits fuer Login und Uploads.
- Explizite Admin- und Teacherrollen.
- Release-Security-Checkliste.
...
Sicherheitskonzept fuer Liveserver beschreibt einen konkreten Baustein von Liveserver. Entscheidend ist, dass Bedienkomfort, Datenmodell, Rechtepruefung und Betrieb zusammenpassen. Wenn diese Grenzen sauber bleiben, kann Liveserver weiter wachsen, ohne unuebersichtlich oder unsicher zu werden.

## 15. Detaillierte technische Leitlinien

Fuer `Security` gilt, dass jede technische Aenderung zuerst aus der bestehenden Liveserver-Struktur heraus gedacht werden muss. Die Anwendung ist absichtlich einfach aufgebaut: HTML-Dateien liefern die Oberflaeche, JavaScript ruft PHP-Endpunkte auf, PHP prueft Berechtigungen und SQLite speichert den fachlichen Zustand. Diese Einfachheit ist ein Vorteil, solange neue Funktionen nicht an den vorhandenen Helfern vorbeigebaut werden.
...
Eine sinnvolle Teststrategie fuer `Security` besteht aus positiven und negativen Tests. Positive Tests pruefen, dass der normale Nutzerfluss funktioniert. Negative Tests pruefen, dass unerlaubte Zugriffe scheitern. Negative Tests sind hier besonders wichtig, weil die Anwendung viele direkte PHP-Endpunkte hat.
...
Fuer den Betrieb ist `Security` nur dann sauber, wenn Dokumentation und ausgelieferter Stand zusammenpassen. Die Version in `config/settings.json`, README und Asset-Querystrings sollte synchron sein. Wenn ein Konzeptverhalten geaendert wird, sollte entweder die passende Konzeptdatei oder die README angepasst werden. Wenn sich Sicherheitsverhalten aendert, muss das Security-Konzept mitgezogen werden.
