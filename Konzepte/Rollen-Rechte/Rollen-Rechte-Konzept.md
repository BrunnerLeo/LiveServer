# Rollen- und Rechtekonzept fuer Liveserver
...
Das Rollen- und Rechtekonzept trennt projektbezogene Berechtigungen von zukuenftigen Schulrollen wie Lehrer und Admin.
...
- `users.role` existiert mit Default `student`.
- Projektrechte laufen ueber Owner, Visibility und Shares.
- Editorrechte laufen ueber `read`/`write`.
- Adminbereich existiert noch nicht vollstaendig.
- Teacherfunktionen sind geplant.
...
- Studenten verwalten eigene Projekte.
- Lehrer koennen spaeter Klassen verwalten.
- Admins koennen System verwalten.
- Rollen ersetzen nicht Projektrechte.
- Server prueft jede Rollenaktion.
...
1. Login liefert Rolle.
2. UI blendet Optionen ein.
3. Backend prueft Rolle.
4. Projektrecht wird separat geprueft.
5. Audit protokolliert privilegierte Aktionen.
...
- Clientrolle nicht vertrauen.
- Nur Admin setzt Rollen.
- Teacherrechte eng begrenzen.
- Public-Write nicht mit Admin verwechseln.
- Rollenwechsel kann Sessions betreffen.
...
- Student kein Admin-Endpunkt.
- Ownerrechte funktionieren.
- Shared-Write nur Editor.
- Rolle in Sessioncheck korrekt.
- Direktaufruf wird blockiert.
...
Rollen- und Rechtekonzept fuer Liveserver beschreibt einen konkreten Baustein von Liveserver. Entscheidend ist, dass Bedienkomfort, Datenmodell, Rechtepruefung und Betrieb zusammenpassen. Wenn diese Grenzen sauber bleiben, kann Liveserver weiter wachsen, ohne unuebersichtlich oder unsicher zu werden.

## 15. Detaillierte technische Leitlinien

Fuer `Rollen-Rechte` gilt, dass jede technische Aenderung zuerst aus der bestehenden Liveserver-Struktur heraus gedacht werden muss. Die Anwendung ist absichtlich einfach aufgebaut: HTML-Dateien liefern die Oberflaeche, JavaScript ruft PHP-Endpunkte auf, PHP prueft Berechtigungen und SQLite speichert den fachlichen Zustand. Diese Einfachheit ist ein Vorteil, solange neue Funktionen nicht an den vorhandenen Helfern vorbeigebaut werden.
...
Eine sinnvolle Teststrategie fuer `Rollen-Rechte` besteht aus positiven und negativen Tests. Positive Tests pruefen, dass der normale Nutzerfluss funktioniert. Negative Tests pruefen, dass unerlaubte Zugriffe scheitern. Negative Tests sind hier besonders wichtig, weil die Anwendung viele direkte PHP-Endpunkte hat.
...
Fuer den Betrieb ist `Rollen-Rechte` nur dann sauber, wenn Dokumentation und ausgelieferter Stand zusammenpassen. Die Version in `config/settings.json`, README und Asset-Querystrings sollte synchron sein. Wenn ein Konzeptverhalten geaendert wird, sollte entweder die passende Konzeptdatei oder die README angepasst werden. Wenn sich Sicherheitsverhalten aendert, muss das Security-Konzept mitgezogen werden.
