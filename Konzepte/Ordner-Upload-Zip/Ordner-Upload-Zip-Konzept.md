# Konzept Ordner-Upload und ZIP fuer Liveserver
...
Dieses Konzept beschreibt, wie Liveserver mehrdateilige Projektstrukturen speichert, im Editor nutzt und als ZIP wieder ausgibt.
...
- `project_files.relative_path` speichert sichtbare Pfade.
- `project_folders` speichert bekannte Ordner.
- `collect_project_folder_paths` ermittelt Ordner.
- `download_project.php` erzeugt ZIP-Eintraege.
- `editor.js` rekonstruiert Dateibaum und Ordner.
...
- Ordnerstruktur bleibt sichtbar.
- Physische Speicherung bleibt flach und sicher.
- Datei-Projekte koennen als ZIP geladen werden.
- Webpage-Projekte behalten Assetpfade.
- Leere Ordner koennen im Editor modelliert werden.
...
- `collect_parent_folder_paths`
- `replace_project_folders`
- `safe_zip_entry_name`
- `project_webpage_root_prefix`
- `project_webpage_public_path`.
...
1. Frontend sendet relative Datei- und Ordnerpfade.
2. Backend bereinigt Pfade.
3. Dateien werden gespeichert.
4. Ordner werden in `project_folders` ersetzt.
5. ZIP oder Site-Routing nutzt dieselbe Struktur.
...
- Keine `..`-Segmente.
- ZIP-Slip verhindern.
- Nutzerpfade nie als Serverpfade verwenden.
- Editorpfade erneut serverseitig bereinigen.
...
- Ordner mit Unterordnern hochladen.
- ZIP entpacken und Struktur pruefen.
- Webpage-Root-Prefix testen.
- Leerer Ordner im Editor pruefen.
- Pfadmanipulation testen.
...
Konzept Ordner-Upload und ZIP fuer Liveserver beschreibt einen konkreten Baustein von Liveserver. Entscheidend ist, dass Bedienkomfort, Datenmodell, Rechtepruefung und Betrieb zusammenpassen. Wenn diese Grenzen sauber bleiben, kann Liveserver weiter wachsen, ohne unuebersichtlich oder unsicher zu werden.

## 15. Detaillierte technische Leitlinien

Fuer `Ordner-Upload-Zip` gilt, dass jede technische Aenderung zuerst aus der bestehenden Liveserver-Struktur heraus gedacht werden muss. Die Anwendung ist absichtlich einfach aufgebaut: HTML-Dateien liefern die Oberflaeche, JavaScript ruft PHP-Endpunkte auf, PHP prueft Berechtigungen und SQLite speichert den fachlichen Zustand. Diese Einfachheit ist ein Vorteil, solange neue Funktionen nicht an den vorhandenen Helfern vorbeigebaut werden.
...
Eine sinnvolle Teststrategie fuer `Ordner-Upload-Zip` besteht aus positiven und negativen Tests. Positive Tests pruefen, dass der normale Nutzerfluss funktioniert. Negative Tests pruefen, dass unerlaubte Zugriffe scheitern. Negative Tests sind hier besonders wichtig, weil die Anwendung viele direkte PHP-Endpunkte hat.
...
Fuer den Betrieb ist `Ordner-Upload-Zip` nur dann sauber, wenn Dokumentation und ausgelieferter Stand zusammenpassen. Die Version in `config/settings.json`, README und Asset-Querystrings sollte synchron sein. Wenn ein Konzeptverhalten geaendert wird, sollte entweder die passende Konzeptdatei oder die README angepasst werden. Wenn sich Sicherheitsverhalten aendert, muss das Security-Konzept mitgezogen werden.
